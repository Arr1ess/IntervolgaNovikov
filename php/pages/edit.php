<div class="container mx-auto p-8">
    <h1 class="text-3xl font-bold mb-6">Edit Entry in Table: <?php echo htmlspecialchars($tablename); ?></h1>
    ...
    <form id="editForm" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <input type="hidden" name="tablename" value="<?php echo htmlspecialchars($tablename); ?>">
        <input type="hidden" name="id" value="<?php echo htmlspecialchars($row[0]['id']); ?>">
        ...
        <?php foreach ($table_columns as $column): ?>
            ...
            <div class="mb-4">
                ...
                <?php if ($column['Field'] == 'type' && isset($enumValues) && is_array($enumValues)): ?>
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="<?php echo htmlspecialchars($column['Field']); ?>" name="<?php echo htmlspecialchars($column['Field']); ?>" required>
                        <?php foreach ($enumValues as $value): ?>
                            <option value="<?php echo htmlspecialchars($value); ?>" <?php echo ($row[0][$column['Field']] == $value) ? 'selected' : ''; ?>><?php echo htmlspecialchars($value); ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <?php elseif (isset($dependedColumns[$column['Field']]) && is_array($dependedColumns[$column['Field']])): ?>
                    <select class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="<?php echo htmlspecialchars($column['Field']); ?>" name="<?php echo htmlspecialchars($column['Field']); ?>" required>
                        <?php foreach ($dependedColumns[$column['Field']] as $option): ?>
                            <option value="<?php echo htmlspecialchars($option); ?>" <?php echo ($row[0][$column['Field']] == $option) ? 'selected' : ''; ?>><?php echo htmlspecialchars($option); ?></option>
                        <?php endforeach; ?>
                    </select>
                ...
                <?php elseif (strpos($column['Type'], 'varchar') !== false || $column['Type'] === 'text'): ?>
                    <input type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="<?php echo htmlspecialchars($column['Field']); ?>" name="<?php echo htmlspecialchars($column['Field']); ?>" value="<?php echo htmlspecialchars($row[0][$column['Field']]); ?>" required>

                <?php elseif ($column['Type'] === 'date'): ?>
                    <input type="date" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="<?php echo htmlspecialchars($column['Field']); ?>" name="<?php echo htmlspecialchars($column['Field']); ?>" value="<?php echo htmlspecialchars($row[0][$column['Field']]); ?>" required>

                <?php elseif ($column['Type'] === 'datetime'): ?>
                    <input type="datetime-local" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" id="<?php echo htmlspecialchars($column['Field']); ?>" name="<?php echo htmlspecialchars($column['Field']); ?>" value="<?php echo htmlspecialchars($row[0][$column['Field']]); ?>" required>
...
<script>
    document.getElementById('editForm').addEventListener('submit', function(event) {
        ...
        fetch('/update', {
            ...
            .then(response => {
                if (response.ok) {
                    window.location.href = '/table?tablename=' + data.tablename;
                } else {
